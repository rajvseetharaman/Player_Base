...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs"
  ...
  <xsl:template name="pageFullView">
    ...
    <xsl:for-each select="//cricketPlayer">
      ...
      <xsl:result-document href="../html/{$itemId}_{$navType}.html" format="html">
        <html>
          <head>
            <title><xsl:value-of select="/cricketPlayerBase/about/title"/>></title>
            <meta charset="utf-8">
              <xsl:text> </xsl:text>
            </meta>
          ...
          <body>
            <div class="page">
              ...
              <xsl:call-template name="bannerArea"></xsl:call-template>
              </div>
        ...
        <body>
          <div class="page">
            ...
            <div class="contentArea">
              ...
              <p>Author: <xsl:value-of select="/cricketPlayerBase/about/author"/></p>
              <xsl:copy-of select="/cricketPlayerBase/about/description"></xsl:copy-of>
              <h3>Information sources</h3>
              <xsl:for-each select="//source">
                <p>Source name: <xsl:value-of select="name"/></p>
                <p>Source URL: <xsl:value-of select="url"/></p>
                <p>Source description: <xsl:value-of select="description"/></p>
                <br></br>
              </xsl:for-each>
            </div>
          </div>
  ...
  <xsl:template name="bannerArea">
    <p><a href="default.html">HOME</a>    |    <a href="about.html">ABOUT</a>    |    <a href="p1_hierarchy.html">HIERARCHY</a>    |    <a href="p1_index.html">INDEX</a>    |    <a href="p1_sequence.html">SEQUENCE</a></p>
  </xsl:template>
  
  <xsl:template name="bannerView">
    <a href="default.html"><img src="images/Cricket_logo.png"><xsl:text> </xsl:text></img></a>
      
    <h1><xsl:value-of select="/cricketPlayerBase/about/title"/></h1>
  </xsl:template>
  
  <xsl:template name="fullView">
    <xsl:param name="itemId"/>
    <xsl:for-each select="//cricketPlayer[@id =$itemId]">
      <img src="images/{photos/photo/@fileName}"><br></br>
        <xsl:text> </xsl:text>
      </img>
      <ul>
  ...
  <xsl:template name="partialView">
    <xsl:for-each select="//cricketPlayer">
      <div class="partialViewArea">
        <a href="{@id}_hierarchy.html"><img class="partialViewImage" src="images/{photos/photo/@fileName}"></img></a><br></br>
        <xsl:text> </xsl:text>
        <p><b><a href="{@id}_hierarchy.html"><xsl:value-of select="title"/></a></b></p>
        <p><b><xsl:value-of select="nationalSide"/></b></p>
        <p><i><xsl:value-of select="playingRole"/></i></p>
      </div>
    </xsl:for-each>  
  </xsl:template>
    ...
    <xsl:for-each select="//cricketPlayer[@id=$itemId]">
      ...
      <p>
        <xsl:for-each select="relatedPlayers/playerRefId">
          ...
          <a href="{$relatedPlayers}_hierarchy.html">
